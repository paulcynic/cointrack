...
class GeckoClient:
    ...
    def _add_response_to_db(self, response: dict, user_id: int, db: Session) -> CoinPriceCreate:
        ...
        coin_price_in = CoinPriceCreate(
                ...
                price = price,
                submitter_id = user_id) # Нужно добавить пользователя полученного из токена
    ...
    def send_telegram_message(self, coin, currency, lower, upper, user_id) -> dict:
        get_params={"ids": coin, "vs_currencies": currency}
        res = self.get_simple_price(params=get_params)
        db = deps.SessionLocal()
        db_obj = self._add_response_to_db(res, user_id, db)
        db.close()
